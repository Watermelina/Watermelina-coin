...
function getSupabase() {
  ...
  if (!url || !key) {
    console.error('[CLAIM-REF-MISSION] CONFIG ERROR: Missing Supabase URL or key');
    return null;
  }
...
const ALLOWED_MISSIONS = ['ref_invite_1', 'ref_invite_3', 'ref_invite_5', 'ref_invite_10', 'ref_invite_15', 'ref_invite_20'];
...
export default async (req) => {
  ...
  console.log(`[CLAIM-REF-MISSION] ── Start ── user=${userId} mission=${mission_key}`);

  try {
    const { data, error } = await supabase.rpc('claim_referral_mission', {
      p_user_id: userId,
      p_mission_key: mission_key
    });

    if (error) {
      console.error(`[CLAIM-REF-MISSION] RPC error: code=${error.code} message=${error.message} details=${error.details}`);
      return Response.json(
    ...
    console.log(`[CLAIM-REF-MISSION] ── Result ── ${JSON.stringify(result)}`);
    return Response.json(result);
  } catch (err) {
    console.error(`[CLAIM-REF-MISSION] Unexpected error: ${err.message}`);
    return Response.json(
...
export const config = {
  path: '/api/claim-referral-mission',
  method: 'POST'
};
